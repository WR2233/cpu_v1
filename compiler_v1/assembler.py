...
def assemble(source: str, file_change_line: List[int] = None, return_line_mapping: bool = False):
    ...
    if return_line_mapping:
        return binary, pc_to_source_line, labels
    else:
        return binary
...
def link_files(input_files, output=None, format='hex', assemble_output=None, pc_mapping_output=None, labels_ret=False):
    ...
    binary, pc_to_line, labels = assemble(merged_source, file_change_line, return_line_mapping=True)
...
def make_pc2label(label: dict[str, int]) -> dict[int, str]:
    res = {}
    for l, pc in label.items():
        if not (l.startswith(".") or l.startswith("_")):
            res[pc] = l
    return res
