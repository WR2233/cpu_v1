...
let rec g dest cont regenv = function (* 命令列のレジスタ割り当て (caml2html: regalloc_g) *)
  | Ans(expp) -> g'_and_restore dest cont regenv expp
  | Let((x, t) as xt, (exp, p), e) ->
      assert (not (M.mem x regenv));
      ...
      | Spill(y) ->
          ...
          (seq((save, p), concat e1' (r, t) e2'), regenv2)
      | Alloc(r) ->
...
and g' dest cont regenv (exp, p) = match exp with (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  ...
  | IfEq(x, y', e1, e2) -> g'_if dest cont regenv (exp, p) (fun e1' e2' -> IfEq(find x Type.Int regenv, find' y' regenv, e1', e2')) e1 e2
  | IfLE(x, y', e1, e2) -> g'_if dest cont regenv (exp, p) (fun e1' e2' -> IfLE(find x Type.Int regenv, find' y' regenv, e1', e2')) e1 e2
  | IfGE(x, y', e1, e2) -> g'_if dest cont regenv (exp, p) (fun e1' e2' -> IfGE(find x Type.Int regenv, find' y' regenv, e1', e2')) e1 e2
  | IfFEq(x, y, e1, e2) -> g'_if dest cont regenv (exp, p) (fun e1' e2' -> IfFEq(find x Type.Float regenv, find y Type.Float regenv, e1', e2')) e1 e2
  | IfFLE(x, y, e1, e2) -> g'_if dest cont regenv (exp, p) (fun e1' e2' -> IfFLE(find x Type.Float regenv, find y Type.Float regenv, e1', e2')) e1 e2
  | CallCls(x, ys, zs) ->
      if List.length ys > Array.length regs - 1 || List.length zs > Array.length fregs then
        ...
        g'_call dest cont regenv (exp, p) (fun ys zs -> CallCls(find x Type.Int regenv, ys, zs)) ys zs
  | CallDir(Id.L(x), ys, zs) ->
      if List.length ys > Array.length regs || List.length zs > Array.length fregs then
        failwith (Format.sprintf "cannot allocate registers for arugments to %s" x)
      else
        g'_call dest cont regenv (exp, p) (fun ys zs -> CallDir(Id.L(x), ys, zs)) ys zs
  | Save(x, y) -> assert false
and g'_if dest cont regenv (exp, p) constr e1 e2 = (* ifのレジスタ割り当て (caml2html: regalloc_if) *)
  ...
  (List.fold_left
     ...
     (fv cont),
   regenv')
and g'_call dest cont regenv (exp, p) constr ys zs = (* 関数呼び出しのレジスタ割り当て (caml2html: regalloc_call) *)
  (List.fold_left
     (fun e x ->
...
let f (Prog(data, fundefs, e)) = (* プログラム全体のレジスタ割り当て (caml2html: regalloc_f) *)
  ...
  let e', regenv' = g (Id.gentmp Type.Unit, Type.Unit) (Ans((Nop, dummy_pos))) M.empty e in
  Prog(data, fundefs', e')
